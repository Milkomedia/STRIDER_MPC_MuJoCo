...
class StriderNMPC:
    def __init__(self):
        ...
        # 2. build "USE-FULL" (current full model)
        ...
        # use_delta results upcasted to use_full dims for mmap (C-contiguous)
        ...
        # Cached holds for use_delta -> use_full upcast (avoid per-tick small allocations).
    ...
    def use_delta_solve(self, x_0, u_0, p, steps_req: int):
        ...
        # Down-project full-model packet (USE_ARM alias / USE_FULL solver path) -> use_delta dimensions
    ...
    def use_arm_solve(self, x_0, u_0, p, steps_req: int):
        # Temporary policy:
        # "USE_ARM" temporarily reuses the same solver/model as "USE_FULL".
        # Keep this wrapper so that the branch structure is already 3-way.
        return self.use_full_solve(x_0, u_0, p, steps_req)
    ...
    def _extract_all_xup(self, full_model_using: bool, steps_req: int = 0, u_opt_out: np.ndarray | None = None, u_rate_out: np.ndarray | None = None,):
        N = self.N
